...
// Handle registers the handler for the giveen message id
func (mux *Mux) Handle(id prot.MessageIdentifier, handler Handler) {
	...
	if mux.m == nil {
		mux.m = make(map[prot.MessageIdentifier]Handler)
	}

	if _, ok := mux.m[id]; ok {
		logrus.Infof("bridge: overwriting bridge handler for type: %d", id)
	}
...
type ResponseWriter interface {
	...
	Write(interface{})
	// Error writes the provide error as a response to the message.
	Error(error)
}
...
//
// 1. Request/Response where by using the `Handler` a request
...
//
// 2. `PublishNotification` where by a notification that was not initiated
...
func (b *Bridge) ListenAndServe() (conerr error) {
	...
	go func() {
		for req := range requestChan {
			go func(r *Request) {
				...
				if !wr.respWritten {
					logrus.Errorf("bridge: request: ID: %d, Type: %d failed to write a response.\n", r.Header.ID, r.Header.Type)
				}
			}(req)
		}
	}()
	// process each bridge response sync this is valid for request/response and publish workflows
	go func() {
	...
	// TODO: We should likely just log a generic response and keep listening?
	select {
